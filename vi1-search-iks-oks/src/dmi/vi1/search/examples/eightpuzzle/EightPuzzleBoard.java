...
public class EightPuzzleBoard {
	...
	public static Action LEFT = new EightPuzzleAction("Left");

	public static Action RIGHT = new EightPuzzleAction("Right");

	public static Action UP = new EightPuzzleAction("Up");

	public static Action DOWN = new EightPuzzleAction("Down");
   ...
   //vraca vrednost koja se nalazi na zadatoj poziciji
	public int getValueAt(XYLocation loc) {
		return getValueAt(loc.getPositionX(), loc.getPositionY());
	}

	//vraca na kojoj lokaciji se nalazi zadata vrednost
	...
	public void setBoard(List<XYLocation> locs) {
		...
		for (int i = 0; i < locs.size(); i++) {
			...
			this.setValue(loc.getPositionX(), loc.getPositionY(), count);
			count = count + 1;
		}
	}
