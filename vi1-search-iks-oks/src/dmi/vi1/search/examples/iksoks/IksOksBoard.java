...
public class IksOksBoard {
	...
	public boolean isEmpty(int row, int col) {
		return state[getAbsPosition(row, col)].equals(EMPTY);
	}
	...
	public boolean isAnyRowComplete() {
		for (int i = 0; i < 3; i++) {
			...
			if (!val.equals(EMPTY) && val.equals(getValue(i, 1)) && val.equals(getValue(i, 2)))
				return true;
		}
		return false;
	}
	...
	public boolean isAnyColumnComplete() {
		for (int j = 0; j < 3; j++) {
			...
			if (!val.equals(EMPTY) && val.equals(getValue(1, j)) && val.equals(getValue(2, j)))
				return true;
		}
		return false;
	}
	...
	public boolean isAnyDiagonalComplete() {
		...
		if (!val.equals(EMPTY) && val.equals(getValue(1, 1)) && val.equals(getValue(2, 2)))
			return true;
		val = getValue(0, 2);
		if (!val.equals(EMPTY) && val.equals(getValue(1, 1)) && val.equals(getValue(2, 0)))
			return true;
		return retVal;
	}
