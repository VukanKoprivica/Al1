...
public class FourInARowBoard {
	...
	private boolean checkHorizontal() {
		for (int j = 0; j < Y_DIMENSION; j++) {
			for (int i = 0; i < X_DIMENSION - 3; i++) {
				if (matrix[j][i] != EMPTY && matrix[j][i] == matrix[j][i+1] && matrix[j][i+1] == matrix[j][i+2] && matrix[j][i+2] == matrix[j][i+3]) {
	...
	public String toString() {
		...
		return sb.toString()
				...
				.replace("0",String.format("%"+ Character.toString(0x1F7E1).length()+"s" ,"  "));
	}	
}
